...
> 🧩 **각 run이 어떤 방법론(후보 생성 + 랭킹 + 다양화 블록)으로 만들어졌는지**는
> [`docs/METHODOLOGY_BREAKDOWN.md`](docs/METHODOLOGY_BREAKDOWN.md)에 모델별로 분해돼 있다.
> (예: P6 = KeyBERT 추출 + KeyBART 생성 → ms-marco-MiniLM 크로스인코더 PRMU 가중 pairwise 재정렬)
...
- [x] **LLM 앙상블 채점표 구축 및 P7 검증 완료** → **§6-5**. 4개 LLM 리랭커(SPECTER2 / gte-ModernBERT / Qwen3 / EmbeddingGemma)의 RRF를 채점표(정답 기준)로 삼아 P7·KeyBART 랭킹 재현 채점 → **P7이 문서 85%에서 우세**.
- [x] **KeyBART ↔ P7 랭킹 비교 완료** → §6-5 (채점표 기준 nDCG@10 P7 0.898 vs KeyBART 0.784).
- [ ] Gold label 결측치(present인데 gold에서 누락된 케이스) 보정 후 재평가
- [ ] indifference margin 기준 P5/P6/P7 간 통계적 동등성 검정 (paired bootstrap)

---

## 6-5. 채점표(앙상블) 기반 P7 검증

6-4의 Open Item(LLM 앙상블 채점표)을 구현·수행한 결과다. 앙상블은 **gold-F1을 두고 겨루는
참가 모델이 아니라, 다른 모델의 랭킹을 채점하는 채점표(정답 기준)**로 쓴다.

**채점표 구성 — 4개 리랭커의 RRF 합의** (모두 P7과 동일 레시피, 인코더만 상이):
...
(팀 지정 5개 중 DeBERTa-v3는 bf16 발산으로 탈락 → 4개. P7은 채점 **대상**이라 채점표에서 제외 = 순환 방지.)

**채점 결과 — 앙상블 채점표 랭킹을 정답으로 P7 vs KeyBART 채점:**

| 지표 (채점표 기준) | P7 | KeyBART | P7 우세 문서 | Wilcoxon p |
| --- | --- | --- | --- | --- |
| nDCG@10 | **0.898** | 0.784 | **85.2%** | ≈0 |
| RBO@10 | **0.452** | 0.325 | **85.1%** | ≈0 |

**해석**: P7 단일 모델이 4-모델 앙상블의 합의 랭킹을 사실상 재현한다(문서 85%에서 베이스라인 압도).
즉 P7은 **앙상블의 배포용 단일 대리자** — 4모델 추론 없이 P7 하나로 채점표 품질에 근접한다. §6-3의
H1(hybrid)·H3(도메인 encoder) 지지에 더해, 7절 타당성 검증(gold 편향 우회)의 정량 근거가 된다.

- 상세: [`docs/ENSEMBLE_REFERENCE_COMPARISON.md`](docs/ENSEMBLE_REFERENCE_COMPARISON.md) · 노트북 [`notebooks/05_ensemble_reference_comparison.ipynb`](notebooks/05_ensemble_reference_comparison.ipynb)
- 채점표 재료(인코더 토너먼트 P8~P15 선정 기록): [`docs/ENCODER_TOURNAMENT_STATUS.md`](docs/ENCODER_TOURNAMENT_STATUS.md)
...
## 8. 구현 및 재현 (Implementation)

본 계획서의 전 실험은 재현 가능한 코드로 구현돼 있다 (본 브랜치에 포함).

| 자산 | 위치 | 내용 |
| --- | --- | --- |
| 🧩 방법론 분해 | [`docs/METHODOLOGY_BREAKDOWN.md`](docs/METHODOLOGY_BREAKDOWN.md) | 각 모델 = 후보 생성 + 랭킹 + 다양화 블록 (헷갈리면 여기부터) |
| 모델 카탈로그 | [`MODEL_CATALOG.md`](MODEL_CATALOG.md) | 모델 ↔ 코드 ↔ 산출물 ↔ 기법 완전 매핑 |
| 핵심 모듈 | `src/` | 단일 평가기 `src/metrics.py`, 전처리·PRMU `src/preprocessing.py` 등 |
| 실행 스크립트 | `scripts/` | B0~B4, P1/P2, P5/P6, P7, 인코더 토너먼트, 채점표 앙상블 |
| 노트북 (출력 포함) | `notebooks/` | 00 결과 · 01 데이터/평가 · 02 베이스라인 · 03 리랭커 · 04 토너먼트 · 05 채점표 |
| 결과·지표 | `results/metrics/` | 리더보드 `grand_comparison_all_models.csv`, 원장 `experiments.csv` 등 |
| 문서별 예측 | `results/predictions_per_document/` | 모델별 top-10 예측 + 정오 대조 (17종) |
| 상세 문서 | `docs/` | 지표 정의·모델 설명·채점표 검증 등 (재현 개요: [`docs/IMPLEMENTATION_OVERVIEW.md`](docs/IMPLEMENTATION_OVERVIEW.md)) |

```bash
pip install -r requirements.txt   # GPU 권장
python scripts/run_experiments.py --profile full     # B0~B4, P1, P2
python scripts/pairwise_reranker.py --stage all      # P5, P6
python scripts/scibert_hybrid_ranker.py              # P7
python scripts/encoder_tournament.py --stage run     # 채점표 재료 P8~P15
python scripts/team_spec_ensemble.py                 # 채점표(앙상블)
```
