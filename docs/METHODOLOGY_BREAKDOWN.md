...
## 1-b. ⚠️ "하이브리드"는 무엇 + 무엇인가 (두 가지 의미)

이 프로젝트에서 **"하이브리드"라는 말이 두 층위에서 쓰인다.** 헷갈리기 쉬우니 구분한다.

### ① 후보 하이브리드 = **KeyBERT(추출형) + KeyBART(생성형)**  ← P1~P7 전부 공통

서로 강점이 다른 두 계열을 **후보 단계에서 합친다**:

| 합치는 모델 | 계열 | 담당 | 강점 |
|---|---|---|---|
| **KeyBERT** (`all-MiniLM-L6-v2`) | 추출형 (extractive) | 본문에 있는 구절 뽑기 | **present**(등장 키프레이즈) 정확 |
| **KeyBART** (`bloomberg/KeyBART`) | 생성형 (generative) | 새 표현 생성 | **absent**(미등장 키프레이즈) 가능 |

→ "추출형 + 생성형" 하이브리드라서 present는 KeyBERT가, absent는 KeyBART가 채운다.
P1의 이름 **"Hybrid 융합"**은 바로 이 **KeyBERT + KeyBART** 후보 하이브리드를 SBERT 점수로 합친다는 뜻이다.

### ② P7의 "Hybrid **Ranker**" = **SciBERT 인코더 + aux 보조피처**  ← P7 랭킹 블록의 이름

P7의 "hybrid"는 후보가 아니라 **랭커 아키텍처**를 가리킨다 — 다른 층위다:

| 합치는 것 | 내용 |
|---|---|
| **SciBERT 인코더 표현** | (문서, 후보)를 함께 읽은 임베딩 |
| **+ aux 보조피처 3개** | 생성 확률 · 생성 여부 · present 여부 (KeyBART 생성 신호) |

→ 인코더 표현과 보조피처를 **MLP에서 결합(hybrid)**해 점수를 낸다.

**정리**: **P7 = 이중 하이브리드**다. 후보는 ①(KeyBERT+KeyBART) 하이브리드, 랭커는 ②(SciBERT+aux) 하이브리드.
P1/P2는 ①만 있고 랭킹은 비학습 융합이다.

---

## 1-c. "Hybrid 융합(fusion)"이 실제로 하는 일 (P1)

가장 헷갈리는 부분. **융합 = 후보 하나하나를 4가지 기준으로 채점해서 가중 합산하는 것**이다.
학습이 전혀 없다 — 그냥 4개 점수를 정해진 비율로 섞어 최종 점수를 매기고, 그 점수로 줄 세운다.

### 한 후보가 받는 4가지 점수 (각각 0~1로 정규화)

| 점수 | 가중치 | 무엇을 재나 | 어디서 나오나 |
|---|---:|---|---|
| **생성 점수** S_gen | **35%** | KeyBART가 이 구절을 **얼마나 확신하며 생성**했나 | KeyBART 생성 확률 |
| **의미 점수** S_sem | **35%** | 이 구절이 **문서 전체 내용과 의미가 통하나** | SBERT로 (문서 ↔ 후보) 코사인 유사도 |
| **추출 점수** S_ext | **20%** | KeyBERT가 본문에서 **얼마나 중요하게 뽑았나** | KeyBERT 유사도 점수 |
| **제목 점수** S_title | **10%** | 이 구절 단어가 **제목에 들어있나** | 제목과 겹치는 토큰 비율 |

### 최종 점수 = 가중 합산

$$
\text{final} = 0.35 \cdot S_{gen} + 0.35 \cdot S_{sem} + 0.20 \cdot S_{ext} + 0.10 \cdot S_{title}
$$

이 점수 내림차순으로 정렬 → 상위 10개가 P1의 예측. (코드: `src/reranking.py`의 `fuse_scores`)

### 예시로 보기 — 논문 "Titans: neural long-term memory…"

| 후보 | 생성(0.35) | 의미(0.35) | 추출(0.20) | 제목(0.10) | **최종** |
|---|---:|---:|---:|---:|---:|
| `long-term memory` | 0.90 | 0.85 | 0.70 | 1.00 | **0.85** ✅ 상위 |
| `algorithm` (일반어) | 0.30 | 0.20 | 0.40 | 0.00 | **0.26** ❌ 하위 |

→ "long-term memory"는 네 기준 모두 높아 상위, "algorithm"은 문서와 안 맞아(의미↓) 제목에도 없어(제목 0) 밀린다.

### 왜 이렇게 섞나 (융합의 의도)

KeyBERT·KeyBART 각각은 약점이 있다:
- KeyBART는 **자신 있게 생성했지만 엉뚱한** 구절을 낼 수 있다 → **의미 점수(S_sem)**가 걸러냄
- KeyBERT는 **본문과 비슷하지만 너무 일반적인** 구절을 뽑을 수 있다 → 생성·제목 점수가 낮아 밀림

→ **여러 기준을 합쳐서, 한 기준만 높은 후보가 아니라 "골고루 좋은" 후보가 위로 오게** 한다. 이게 융합이다.
P2는 여기에 MMR을 더해 상위권 **중복까지** 제거한 것뿐이다.
